...
on the above factors except total bandwidth and time of day. This tells us what 
...
To address this issue, we run multiple trials of replay (10 total), alternating between using a VPN connection ($R_T$) and 
a plain (untunneled) one ($R_U$). By computing statistics over multiple trials of one category ($R_T$ or $R_U$) we 
can control for whether the ISP can differentiate; we assign the variance in these trials as being due to 
factors other than differentiation. Having computed the variance over $R_T$ and $R_U$, we can 
