...
\mypara{PII leaked from each OS} Table~\ref{tab:pii} presents the PII leaked by Android and iPhone apps. 
...
\mypara{PII leaked from same apps on different OSes} We compared the overlap of the top hundred apps for iOS and Android. 26 apps fell into this category, 17 leaked PII. Only 2 apps had the same data leakage in both OSes: TuneIn Player leaked Location and Yahoo! Mail leaked Android ID and an iOS serial number. Another 2 apps both leaked data, but different types: Snapchat leaked Android ID and IMEI but leaked username in iOS and Brightest Flashlight leaked Android ID and leaked location in iOS. Two apps leaked PII only in iOS (Groupon and Pandora), while 11 apps leaked data only in Android. Most Android leaks were  Android ID; however, 4 of these apps leaked location and 1 leaked IMEI. For the top apps, Android leaks more PII than iOS.
...
\mypara{PII leaked over SSL} During our experiments, we observed that PII is also sent over HTTPS.  In the following, we
