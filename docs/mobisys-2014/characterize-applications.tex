...
In Table~\ref{tab:pii}, we present the different PII leaked by Android and iPhone apps. 
...
We compared the overlap of the top hundred apps for iOS and Android. 26 apps fell into this category. Only 2 apps had the same data leakage in both operating systems: TuneIn Player leaked Location and Yahoo! Mail leaked Android ID and an iOS serial number. Another 2 apps both leaked data, but different types: Snapchat leaked Android ID and IMEI but leaked username in iOS and Brightest Flashlight leaked Android ID and leaked location in iOS. 2 apps leaked only in iOS (Groupon and Pandora), while 11 apps leaked data only in Android. However, most of what was leaked in Android was the Android ID, which is not as bad as leaking IMEI, since the ID is reset when the device is wiped. However, 4 of those 11 leaked Location and 1 leaked IMEI, so Android may still be a poorer choice for privacy. 
...
are ads and analytics (A\&A) sites. \drc{Why is this only for IMEI and Android ID? Also, the table is weak, consider cutting.}
