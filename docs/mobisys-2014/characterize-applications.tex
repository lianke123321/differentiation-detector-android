...
\tbd{AR: I have replaced pair with tracker. Our filter is DNS based and we cannot block for a specific app because DNS requests are agnostic to the apps.}
...
PII leaks are not limited to apps found in authorized stores, we observe that malwares aggressively leak the PIIs.
...
\tbd{AR: DAVE: We need to add this in previous sections.}
