...
In \fref{tab:pii}, we present the different PII leaked for both Android and iPhone apps.  We observe that
...
While only one Android app leaked a password in the clear (\tbd{which category?}), 
...
In \fref{fig:springboard-wild}, we observe that Springboard leaked location information for 11 devices in the \mobWild dataset, a maximum of 14 leaks per day was observed for one device. 
...
In \fref{tab:top-trackers}, we present the number of devices in the \mobWild dataset that contact the most popular A\&A sites, an activity that is receiving considerable attention~\cite{roesner:webtrackers,leontiadis:mobileads,vallina-rod:ads}.
...
Given the extensive nature of tracking over mobile devices, we developed a tool, \emph{ConVis} that allows users to visualize their devices' tracking, and the applications that facilitate this tracking, with the help of a web-based interfaces similar to that of Mozilla Collusion~\cite{collusion}. A demo of 
this tool is located at \url{http://goo.gl/A17h9}.
