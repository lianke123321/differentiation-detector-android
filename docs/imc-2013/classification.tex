...
In \fref{tab:classify-http-cert}, we observe that less than 25\% of the HTTPS traffic in Android and iOS contains the fully qualified domain name (FQDN) in the subject of the certificate;
...
\tbd{table for RE match}

Based on the hostname, the web service can be obtained. 
This can be used to get estimate the application.

\tbd{Why only hostname is not bad -- usefull to characterize web services -- users are interested in webservices and application are medium to access webservice}
