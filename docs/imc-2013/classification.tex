...
Unlike HTTP flows, SSL flows provide limited information in the clear that can be used to identify the applications. 
For the traces captured during our controlled experiments, we were able to observe HTTP requests and responses after decrypting the SSL flows bumped with SSL bumping. 
However, because we did not perform SSL bumping for the devices in the \mobWild dataset, we do not observe these requests and responses.
Our focus was therefore on identifying the Web-services responsible for the SSL flows. 
...
We therefore focus our attention on indentifying the Web-services responsible for the HTTPs flows. 
...
In \fref{tab:sslclassify-popular-host} we present the top5 hostnames that were responsible for 66\% of iOS traffic and 54\% of Android by volume in the \mobWild dataset. 
We observe that despite the hostname we cannot uniquely identify the Web-service for some flows.
...
However, the  hostname \emph{fbcdn-photos-a.akamaihd.net} gives an indication that the traffic is due to Facebook.
...
We observe that the iOS devices in our dataset generated a significant number of email flows.
