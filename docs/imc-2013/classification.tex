...
\begin{table} 
     ...
     \end{tabular}
     \end{small}
     \caption{Classification of applications based on \httphost and \useragent.}
     \label{tab:classification-success}
\end{table}

At least one flow going to the web-service, then we identify the flow an belonging to the applications. 
Indeed we observe that flows to the web-service contain the maximum data (more than 75\%) of the traffic volume when the application was active. 
...
%Indeed, we observe that \tbd{} number of Android applications from the third-party store and \tbd{} number of Android applications from the \emph{Google Play} store contacted ads and analytics sites.
We identify 
