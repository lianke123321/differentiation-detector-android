...
In \fref{tab:classify-ssl-traffic} we present a classification of SSL traffic based on names identified using the SSL fields and DNS queries and port numbers.
We observe that the iOS devices in our dataset generated a significant number of flows to email sites.
Similarly, we were able to label 12.49\% of iOS traffic and 36.661\% to social network services including \emph{google plus, facebook, and twitter}.
...
This low share is because google can use hosts matching the pattern \emph{client*.google.com} to fetch data for different applications and services; we observed a similar behavior in our controlled experiments. 
We therefore group such traffic as traffic from google services.
Indeed, traffic from google is the largest source of SSL traffic for iOS and Android devices in our dataset. 
