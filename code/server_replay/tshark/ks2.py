...
def run(pcap_file):
    ...
    return xputs
def ks2(pcap_file1, pcap_file2):
    xputs1 = run(pcap_file1)
    xputs2 = run(pcap_file2)
    return stats.ks_2samp(xputs1, xputs2)
...
def do_dir(dir):
    ...
    for file in files:
        if file.endswith(".pcap") and '_novpn_' in file:
            novpn_file = dir_path + '/' + file
            number = (file.partition('_novpn_')[2]).partition('_')[0]
            for file in files:
                if ('_vpn_' + number) in file:
                    vpn_file = dir_path + '/' + file
                    break
            res[number] = ks2(novpn_file, vpn_file)
    return res
def main():
#    print ks2(sys.argv[1], sys.argv[2])
#    sys.exit(0)
