...
class Reader(object):
    ...
    def __init__(self, fileobj):
        ...
        #test if EOF has been reached. If this is the case, then it means 
        #that the pcap file terminated abruptly, thus its last entry is not
        #a valid packet entry and we should skip the NeedData exception and
        #raise an exception specific to a partial packet
        try:
            self.__fh = FileHdr(buf)
        except dpkt.NeedData:
            if len(self.__f.read()) != 0:
                raise dpkt.NeedData('End of file not yet reached')
            else:
                raise EndOfFile()
        self.__ph = PktHdr
    ...
    def __iter__(self):
        ...
        while 1:
            ...
            #test if EOF has been reached. If this is the case, then it means 
            #that the pcap file terminated abruptly, thus its last entry is not
            #a valid packet entry and we should skip this exception and simply 
            #stop the iteration. 
            try:
                hdr = self.__ph(buf)
            except dpkt.NeedData:
