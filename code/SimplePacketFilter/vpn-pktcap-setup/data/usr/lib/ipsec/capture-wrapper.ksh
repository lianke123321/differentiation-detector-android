...
startcapture () { 
    ...
    ${TCPDUMP_BIN} -i ${devCapture} -n ip host ${clientIP} -w - | ${GPG_BIN} --homedir=${GPG_HOME} -o ${dumpName} --keyring ${encPublicKeyRing} -er ${encPublicKeyID} --trust-model always > /dev/null 2>&1 
}
